        ...
        let workDetail = this.db.object(dbPath).valueChanges().subscribe((data) => {
          ...
          if (data != null) {
            ...
            if (wardDetails != undefined) {
              ...
              let workerDetails = this.db.object(workerDetailsPath).valueChanges().subscribe((workerData) => {
              ...
              for (let p = 1; p < lineStatus.length - 1; p++) {
                if (lineStatus[p] != null) {
                  if (lineStatus[p]["start-time"] != "undefined" && lineStatus[p]["start-time"] != null) {
                    ...
                    lineStatus[p]["position"] = parseFloat(
                    ...
                    if (data["task" + i + ""]["task"] == zoneNo) {
                      if (data["task" + i + ""]["in-out"] != null) {
                        if (startTime == "") {
                          ...
                          let duration =
                            this.commonService.timeDifferenceMin(new Date(date1), new Date(date2));
                          let cost = ((Number(duration) / 60) * this.totalSalary).toFixed(2);
                          this.commonService.getEmplyeeDetailByEmployeeId(driverId.toString()).then((employee) => {
                            this.db.object("WasteCollectionInfo/" + zoneNo + "/" + year + "/" + monthName + "/" + this.selectedDate + "/Summary").update({ dutyInTime: startTime, wardReachedOn: reachTime, wardReachingDuration: duration, wardReachingCost: cost, });
                            this.db.object("WasteCollectionInfo/" + zoneNo + "/" + year + "/" + monthName + "/" + this.selectedDate + "/WorkerDetails").update({ driverName: employee["name"], });
                            let zoneDetails = this.workList.find((item) => item.zoneNo == zoneNo);
                            if (zoneDetails != undefined) {
                              ...
                              zoneDetails.driverName = employee["name"];
                            }
                          });
                          break;
                        }
