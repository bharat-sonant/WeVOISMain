...
@Component({
  selector: 'app-secondary-collection-ward-mapping',
  templateUrl: './secondary-collection-ward-mapping.component.html',
  styleUrls: ['./secondary-collection-ward-mapping.component.scss']
})
export class SecondaryCollectionWardMappingComponent implements OnInit {
  ...
  getDustbinWardMapping() {
    this.dustbinService.getOpenDepotWardMappingJson().then((mapData: any) => {
      let dustbinMapList = [];
      if (mapData != null) {
  ...
  updateDustbinWardMappingJson() {
    ...
    this.commonService.saveJsonFile(obj, "mappingOpenDepotWard.json", "/DustbinData/");
  ...
  getDustbinList(dustbinMapList: any) {
    this.allDustbinList = [];
    this.allDustbinList = JSON.parse(localStorage.getItem("openDepot"));
