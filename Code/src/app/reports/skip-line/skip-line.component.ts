...
export class SkipLineComponent implements OnInit {
  ...
  constructor(public db: AngularFireDatabase, private mapService: MapService, private commonService: CommonService, private httpService: HttpClient, ) { }
  index: any;
  studentDetail: any;
  zoneList: any[];
  ...
  getSkipLines() {
    ...
    for (let index = 0; index < this.zoneList.length; index++) {
      ...
      let lineData = this.db.list(dbPath).valueChanges().subscribe(
        data => {
          ...
          if (data.length > 0) {
            ...
            for (let j = 0; j < data.length; j++) {
              if (data[j]["Status"] == "skip") {
                ...
                this.skippedLines[i - 1].lines.push({ lineNo: 'Line '+ (j + 1), reason: data[j]["Reason"] });
              }
            }
