  ...
  getAverageData() {
    ...
    if (this.vehicleList.length > 0) {
      for (let i = 0; i < this.vehicleList.length; i++) {
        let days = this.commonService.getDaysBetweenDates(
          "2021-01-01",
          this.toDayDate
        );
      ...
      setTimeout(() => {
        if (this.vehicleDataList.length > 0) {
          ...
          for (let i = 0; i < this.vehicleList.length; i++) {
            ...
            if (vehicleDetails != undefined) {
              ...
              for (let j = 0; j < this.vehicleDataList.length; j++) {
                if (this.vehicleList[i]["vehicle"] == this.vehicleDataList[j]["vehicle"]) {
                  dataList.push({
                    ...
                    amount: this.vehicleDataList[j]["amount"],
                  });
                }
              }
              ...
              vehicles.push({
                ...
                petrol: totalPetrol,
                average: average,
                petrolConsumption: Number(totalPetrol.toFixed(2)),
          ...
          this.vehiclePetrolList = this.commonService.transform(
            vehicles,
            "-petrolConsumption"
          );
    ...
    if (this.vehicleDataList.length > 0) {
      for (let i = 0; i < this.vehicleDataList.length; i++) {
        if (this.vehicleDataList[i]["vehicle"] == vehicle) {
          this.vehicleDetailList.push({
            date: this.vehicleDataList[i]["date"],
            liters: this.vehicleDataList[i]["petrol"],
            km: this.vehicleDataList[i]["km"],
          });
        }
      }
      this.vehicleDetailList = this.vehicleDetailList.sort((a, b) =>
        b.date < a.date ? 1 : -1
      );
    }
      ...
      if (this.vehicleAllPetrolList.length > 0) {
        if (vehicle == "ALL") {
          ...
          for (let i = 0; i < this.vehicleAllPetrolList.length; i++) {
            ...
            this.totalAmount =
              this.totalAmount + Number(this.vehicleAllPetrolList[i]["amount"]);
          }
        } else {
          for (let i = 0; i < this.vehicleAllPetrolList.length; i++) {
            if (this.vehicleAllPetrolList[i]["vehicle"].toString().includes(vehicle)) {
              this.vehiclePetrolList.push({
                ...
                petrol: this.vehicleAllPetrolList[i]["petrol"],
                average: this.vehicleAllPetrolList[i]["average"],
  ...
  getShortData(shortOption: any) {
    ...
    if (this.shortType == "desc") {
      ...
      if (this.shortOption != "vehicle") {
        ...
        this.vehiclePetrolList = this.vehiclePetrolList.sort((a, b) =>
          a.vehicle > b.vehicle ? 1 : -1
        );
      }
    } else {
      this.shortType = "desc";
