...
export class DustbinAnalysisComponent implements OnInit {
  ...
  getAssignedPlans() {
    this.planList = [];
    let assignedPlanPath = this.db.list('DustbinData/DustbinAssignment/' + this.currentYear + "/" + this.currentMonthName + "/" + this.selectedDate).valueChanges().subscribe(
      assignedPlans => {
  ...
  getBinsForSelectedPlan(planId: string) {
    ...
    let pickingPlanPath = this.db.object("DustbinData/DustbinPickingPlans/" + planId).valueChanges().subscribe(
      pickingPlanData => {

        if (pickingPlanData == null) {
          // now need to find with date
          let pickingPlanWithDatePath = this.db.object("DustbinData/DustbinPickingPlans/" + this.selectedDate + "/" + planId).valueChanges().subscribe(
            pickingPlanWithDateData => {
              if (pickingPlanWithDateData == null) {
                let pickingPlanHistory = this.db.object("DustbinData/DustbinPickingPlanHistory/" + this.currentYear + "/" + this.currentMonthName + "/" + this.selectedDate + "/" + planId).valueChanges().subscribe(
                  dustbinPlanHistoryData => {
  ...
  getBinsDetail(bins: string, planId: string) {
    ...
    for (let index = 0; index < binsArray.length; index++) {
      ...
      let binsDetailsPath = this.db.object("DustbinData/DustbinDetails/" + binId + "/address").valueChanges().subscribe(
        dustbinAddress => {
  ...
  setIconClass(binData: any) {
    ...
    if (binData != null) {
      ...
      if (binData["Image"] == undefined) {
        ...
        let dustbinNotFoundImage = binData["Image"]["Urls"]["dustbinNotFoundImageUrl"];
  ...
  setRemark() {
    ...
    if (chkRemark2.checked == false) {
      let isBrokenPath = this.db.object("DustbinData/DustbinDetails/" + this.binDetail.binId + "/isBroken").valueChanges().subscribe(
        isBroken => {
          let chkRemark2 = <HTMLInputElement>document.getElementById('chkRemark2');
  ...
  updatePendingAnalysis() {
    ...
    if (this.binDetail.analysisAt == "") {
      let pendingCountPath = this.db.object("DustbinData/TotalDustbinAnalysisPending").valueChanges().subscribe(
        pedingCount => {
  ...
  saveDustbinAnalysis() {
    ...
    if (this.binDetail.canDoAnalysis == 'yes') {
      this.db.object("DustbinData/DustbinPickHistory/" + this.currentYear + "/" + this.currentMonthName + "/" + this.selectedDate + "/" + this.binDetail.binId + "/" + this.planDetail.planId + "/Analysis/").update({
  ...
  updateIsDustbinBroken() {
    ...
    this.db.object("DustbinData/DustbinDetails/" + this.binDetail.binId + "/").update({
