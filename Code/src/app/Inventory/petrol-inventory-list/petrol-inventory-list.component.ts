...
export class PetrolInventoryListComponent implements OnInit {
  ...
  setNewDataStructure() {
    ...
    for (let j = 1; j <= days; j++) {
      let monthDate =
        currentYear + "-" + currentMonth + "-" + (j < 10 ? "0" : "") + j;
      let dbPath =
  ...
  getPetrolList(vehicle: any) {
    ...
    if ($("#txtDate").val() != "") {
      ...
      let rowTo = days;
      if (
        this.selectedMonth == this.commonService.setTodayDate().split("-")[1]
      ) {
        rowTo = parseInt(this.commonService.setTodayDate().split("-")[2]);
      }
      for (let j = 1; j <= rowTo; j++) {
              ...
              if (keyArray.length > 0) {
                for (let j = 0; j < keyArray.length - 1; j++) {
                  ...
                  if (data[index]["isDelete"] == 0) {
                    ...
                    this.totalLiters =
                      this.totalLiters + Number(data[index]["liters"]);
                    this.totalAmount =
                      this.totalAmount + Number(data[index]["amount"]);
                    let slipImageURL =
