...
export class PetrolInventoryListComponent implements OnInit {
  ...
  deleteEntry(entryNo: any, date: any, vehicleNo: any) {
    ...
    this.db.object("Inventory/PetrolData/" + year + "/" + monthName + "/" + date + "/" + vehicleNo + "/" + entryNo).update({ isDelete: 1, });
  ...
  setVehicleAverage(vehicleNo: any, year: any, monthName: any, month: any) {
    ...
    setTimeout(() => {
      if (this.averageList.length > 0) {
        ...
        this.db.object("Inventory/PetrolData/" + year + "/" + monthName + "/Vehicles/" + vehicleNo).update({ average: avgrage, km: Km, petrol: petrol, totalKm: totalKm, totalPetrol: totalPetrol, });
      }
    }, 2000);
  }
            ...
            if (keyArray.length > 0) {
              for (let j = 0; j < keyArray.length - 1; j++) {
                ...
                if (data[index]["isDelete"] == 0) {
                  ...
                  this.petrolList.push({
                    entryNo: index,
                    km: km,
                    date: monthDate,
                    vehicleNo: this.vehicleAllList[i]["vehicle"],
                    liters: data[index]["liters"],
                    price: Number(data[index]["price"]).toFixed(2),
                    amount: data[index]["amount"],
                    userId: this.userId,
                    createdBy: data[index]["userId"],
